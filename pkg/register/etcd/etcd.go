...
func (e *EtcdRegister) keepliveChan(ctx context.Context) {
	for {
		...
		case <-e.keepaliveChan:
			...
			// 	retry := 0
			// 	for {
			// 		if retry >= maxReconnectAttempts {
			// 			logger.Error(ctx, "etcd keepalive channel closed and max reconnect attempts reached, giving up", zap.Int64("leaseID", int64(e.leaseID)), zap.Int("attempts", retry))
			// 			return
			// 		}
			// 		logger.Warn(ctx, "etcd lease keepalive channel closed, attempting to reestablish", zap.Int64("leaseID", int64(e.leaseID)), zap.Int("attempt", retry+1))
			// 		newKeepaliveChan, err := e.client.KeepAlive(ctx, e.leaseID)
			// 		if err != nil {
			// 			logger.Error(ctx, "etcd keepalive reacquire failed", zap.Error(err), zap.Int("attempt", retry+1))
			// 			retry++
			// 			time.Sleep(2 * time.Second)
			// 			continue
			// 		}
			// 		e.keepaliveChan = newKeepaliveChan
			// 		break
			// 	}
			// 	continue
			// } else {
