...
6. 查看trace链路的调用时长,是哪个请求的时长特别长,查看grand里面关于goruntine和gc的指标
打开pprof查看火焰图,这种一般是DB或者redis连接数造成的,比如链接没有关闭
7. MQ还没复习,但是在写的时候可以make(chan int,1000),定义chat的长度,写入满了就等待,这样可以降低MQ的压力
8. 服务端增加限流,客户端增加熔断操作,重试次数固定而不是一直重试
9. redis使用分布式锁 redis存 key=>value value是一个随机生成的数字,key是根据uid或者其他唯一标识,
ttl 一般五分钟即可  如果一个请求五分钟都没处理 那就是有问题 最后为了强一致性,DB将value设置唯一健
10. 当达到降级的时候  返回默认的数据给前端即可,然后赶快查看服务为什么降级
