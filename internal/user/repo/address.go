...
func (r *Repo) Save(ctx context.Context, userAddress *domain.UserAddress) error {
	...
	if err == nil {
		// 记录已存在，返回错误
		return xerr.New(codes.Internal, fmt.Sprintf("address already exists for user_id=%d, chain=%s", userAddress.UserID, userAddress.Chain))
	}
	if !errors.Is(err, gorm.ErrRecordNotFound) {
		// 其他数据库错误
		return xerr.New(codes.Internal, fmt.Sprintf("check existing address failed: %v", err))
	}
	...
	if err != nil {
		if errors.Is(err, gorm.ErrDuplicatedKey) {
			return xerr.New(codes.Internal, "地址已存在")
		}
		return xerr.New(codes.Internal, fmt.Sprintf("save address failed: %v", err))
	}
	return nil
}
...
func (r *Repo) GetByUserID(ctx context.Context, userID int64) ([]*domain.UserAddress, error) {
	...
	if err != nil {
		return nil, xerr.New(codes.Internal, fmt.Sprintf("get addresses by user id failed: %v", err))
	}
...
func (r *Repo) GetByUserIDAndChain(ctx context.Context, userID int64, chain string) (*domain.UserAddress, error) {
	...
	if err != nil {
		...
		return nil, xerr.New(codes.Internal, fmt.Sprintf("get address by user id and chain failed: %v", err))
	}
	return &address, nil
}
...
func (r *Repo) GetUserIDByAddress(ctx context.Context, address string) (int64, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return 0, xerr.New(codes.Internal, "地址无对应用户")
		}
		return 0, xerr.New(codes.Internal, fmt.Sprintf("get user id by address failed: %v", err))
	}
